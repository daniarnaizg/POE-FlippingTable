...
$.getJSON('https://cors.io/?https://api.poe.watch/get?league=Synthesis&category=currency', function (data) {

	var exPrice = Math.round(data[10]['median']);
	var exIcon = data[10]['icon'];
	var cIcon = data[0]['icon'];
	...
	for (var i = 0; i < data.length; i++) {
		...
		try {
			var tempArray = [];
			var icon_index = data[i]['icon'];
			tempArray.push(icon_index);

			var name = data[i]['name'];
			tempArray.push(name);
			var bp = Math.round(1/data[i]['exalted']);							// BUYING PRICE
			tempArray.push(bp);
			var ratio = data[i]['median']; 										// RATIO
			tempArray.push(ratio);

			// test
			var p_exalted = data[i]['exalted'];
			var ppex = ratio / p_exalted - exPrice;
			// var ppex = bp * ratio - exPrice; 									// PROFIT PER EXALT
			tempArray.push(ppex);

			var daily = data[i]['daily'];
		...
		if (name == 'Exalted Orb' || bp <= 0 || ppex > exPrice / 3 || group == 'piece' || group == 'essence' ){
			continue;
		}
